...
const POSITION_STORAGE_KEY = "mie-fishing-map-position-overrides-v3";
const PREVIOUS_POSITION_STORAGE_KEYS = [
  "mie-fishing-map-position-overrides-v2",
  "mie-fishing-map-position-overrides-v1"
];
const AUTO_ALIGN_STORAGE_KEY = "mie-fishing-map-auto-align-v3";
...
function loadPositionOverrides() {
  ...
  // v32 以前の自動補正は、表記と違う候補を保存してしまうことがあったため引き継ぎません。
...
const GSI_AUTO_ALIGN_VERSION = "v33";
...
function labelLooksLikeWrongType(spot, labelNorm) {
  ...
  if (spot.type === "港" && !/(港|漁港|浦|泊|岸壁|埠頭|ふ頭|防波堤|マリーナ)$/u.test(labelNorm)) return true;
...
async function findGsiLabelPosition(spot, thorough = false) {
  // 「地図に表記されている名前」に合わせるため、地図注記ベクトルを先に見ます。
  const candidates = await lookupGsiVectorLabelCandidates(spot, thorough ? 6 : 2);
  if (thorough || !candidates.some((candidate) => candidate.score >= 145)) {
    candidates.push(...await lookupGsiNameSearchCandidates(spot, thorough));
  }
