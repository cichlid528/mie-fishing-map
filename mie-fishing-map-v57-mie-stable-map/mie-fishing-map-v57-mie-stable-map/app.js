(() => {
  ...
  const POSITION_STORAGE_KEY = "mie-fishing-map-position-overrides-v57";
  const LEGACY_SINGLE_KEY = "mieFishingMap.v1";

  // v57: 地図バグを避けるため、強すぎるマスク・タイル制限を撤去。
  ...
  const MIE_HOME_BOUNDS = [[33.72, 135.80], [35.30, 137.06]];
  const MIE_NAV_BOUNDS = [[33.45, 135.50], [35.55, 137.35]];
  ...
  function resetMieView() {
    if (!map || typeof L === "undefined") return;
    map.fitBounds(getMieHomeBounds(), { padding: [22, 22], animate: false });
    if (map.getZoom() < MIE_MIN_ZOOM) map.setZoom(MIE_MIN_ZOOM, { animate: false });
  }

  function addMieBoundaryLayer() {
    if (!map || typeof L === "undefined") return;
    L.rectangle(getMieHomeBounds(), {
      color: "#0f7b63",
      weight: 2,
      opacity: 0.45,
      fill: false,
  ...
  function setSpotMode(value) {
    ...
    els.dataStatus.textContent = state.spotMode ? "地図をタップして釣り場を追加します。" : "v57・三重県マップ修正版";
  }
  ...
  function setCatchMode(value) {
    ...
    els.addSpotMode.classList.toggle("is-active", state.spotMode);
    els.addCatchMode.classList.toggle("is-active", state.catchMode);
    els.dataStatus.textContent = state.catchMode ? "地図をタップして記録ピンを追加します。" : "v57・三重県マップ修正版";
  }
  ...
  function init() {
    ...
    els.dataStatus.textContent = `v57・三重県マップ修正 / 釣り場${state.spots.length}件 / 記録${state.catches.length}件 / 40up${state.catches.filter(isBigBass).length}件`;
  }
