...
const POSITION_STORAGE_KEY = "mie-fishing-map-position-overrides-v2";
const LEGACY_POSITION_STORAGE_KEY = "mie-fishing-map-position-overrides-v1";
const AUTO_ALIGN_STORAGE_KEY = "mie-fishing-map-auto-align-v2";
...
function loadPositionOverrides() {
  ...
  // v31 以前の自動補正は、間違った候補が端末に残ることがあったため引き継ぎません。
  // ユーザーが詳細カードから手で直した位置だけ、新しい保存先へ移します。
  const legacy = safeParseJson(localStorage.getItem(LEGACY_POSITION_STORAGE_KEY), {});
  const manualOnly = Object.fromEntries(
    Object.entries(legacy).filter(([, value]) => value?.source === "手動補正" && validPosition(value))
  );
...
const GSI_AUTO_ALIGN_VERSION = "v32";
...
function scoreLabelMatch(spot, label) {
  ...
  if (labelNorm === fullName) return 150;
  if (labelNorm.startsWith(fullName) || labelNorm.endsWith(fullName)) return 108;
  if (labelNorm.includes(fullName)) return 92;
  if (fullName.includes(labelNorm) && labelNorm.length >= 3) return 88;
  ...
  variants.forEach((variant) => {
    ...
    if (labelNorm.startsWith(variant) || labelNorm.endsWith(variant)) best = Math.max(best, 86);
    if (labelNorm.includes(variant) || variant.includes(labelNorm)) best = Math.max(best, 72);
  });
  return best;
}
...
async function lookupGsiVectorLabelCandidates(spot, radius = 1) {
  ...
  for (let ring = 0; ring <= radius; ring += 1) {
    for (let dx = -ring; dx <= ring; dx += 1) {
      for (let dy = -ring; dy <= ring; dy += 1) {
        if (Math.max(Math.abs(dx), Math.abs(dy)) === ring) tileOffsets.push([dx, dy]);
      }
    }
  }
  ...
  for (const layer of GSI_VECTOR_TILE_LAYERS) {
    for (const [dx, dy] of tileOffsets) {
      const features = await fetchGsiVectorTile(layer.id, center.x + dx, center.y + dy);
      candidates.push(...candidatesFromFeatures(spot, features, layer.source));
      if (candidates.some((candidate) => candidate.score >= 128)) {
        return candidates.sort((a, b) => b.score - a.score || a.distance - b.distance);
      }
    }
  }
...
async function findGsiLabelPosition(spot, thorough = false) {
  ...
  if (thorough || !candidates.some((candidate) => candidate.score >= 120)) {
    candidates.push(...await lookupGsiVectorLabelCandidates(spot, thorough ? 6 : 2));
  }
