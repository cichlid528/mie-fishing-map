...
const POSITION_STORAGE_KEY = "mie-fishing-map-position-overrides-v6";
...
const AUTO_ALIGN_STORAGE_KEY = "mie-fishing-map-auto-align-v6";
...
const GSI_AUTO_ALIGN_VERSION = "v40";
...
const ZOOM_LABEL_ALIGN_THROTTLE_MS = 900;
...
function rankViewportCandidate(candidate) {
  ...
  return candidate.score - Math.min(currentDistance, 10) * 2 - Math.min(pixelDistance, 1200) / 28;
}
...
async function lookupVisibleGsiLabelCandidate(spot) {
  ...
  for (const zoom of GSI_VECTOR_TILE_ZOOMS) {
    const tiles = tileRangeForMapBounds(bounds, zoom, 1);
    for (const layer of GSI_VECTOR_TILE_LAYERS) {
...
async function alignVisibleSpotsToZoomLabels() {
  ...
  const targets = spots
    .filter((spot) => !spot.custom && !isManualPositionOverride(spot.id) && bounds.contains([spot.lat, spot.lng]))
    .sort((a, b) => {
      if (a.id === selectedId) return -1;
      if (b.id === selectedId) return 1;
      return distanceKm(map.getCenter().lat, map.getCenter().lng, a.lat, a.lng)
        - distanceKm(map.getCenter().lat, map.getCenter().lng, b.lat, b.lng);
    })
    .slice(0, selectedSpot ? ZOOM_LABEL_ALIGN_MAX_TARGETS : Math.min(12, ZOOM_LABEL_ALIGN_MAX_TARGETS));

  let adjusted = 0;
  try {
    for (const spot of targets) {
      const zoomBucket = Math.min(18, Math.max(ZOOM_LABEL_ALIGN_MIN_ZOOM, Math.floor(map.getZoom())));
      const attemptKey = `${spot.id}:${zoomBucket}:${map.getBounds().toBBoxString()}`;
      if (zoomLabelAlignAttempts.has(attemptKey)) continue;
    ...
    if (adjusted > 0) {
      ...
      if (dataStatus) dataStatus.textContent = `拡大表示中の地図名に合わせて${adjusted}件のピンを補正しました`;
    }
  } finally {
