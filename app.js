(() => {
  ...
  const PATCH_VERSION = "v152-pet-mobile-icon-bigger";
  const PATCH_STATUS_LABEL = "v152・釣りニャンスマホ表示さらに大きめ版";
  ...
  const SOURCE_CACHE_KEY = "mie-fishing-map-source-cache-486490f1-v152";
  ...
  function showLoadError(error) {
    console.error("Mie Fishing Map v152 loader failed", error);
  ...
  function patchSource(source) {
    let patched = source
      ...
      .replace('// v131: 中勢グリーンパーク池も除いた初期収録。', '// v152: スマホの釣りニャン表示をさらに見やすい大きさへ調整。')
  ...
  // v152: 釣りニャンの「釣果記録」ボタンから、アプリ本体の釣果記録画面をすぐ開くための橋渡し。
