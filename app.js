(() => {
  ...
  function setSpotMode(value) {
    ...
    els.dataStatus.textContent = state.spotMode ? "地図をタップして釣り場を追加します。" : "v51・緊急修正版";
  }
  ...
  function setCatchMode(value) {
    ...
    els.addSpotMode.classList.toggle("is-active", state.spotMode);
    els.addCatchMode.classList.toggle("is-active", state.catchMode);
    els.dataStatus.textContent = state.catchMode ? "地図をタップして記録ピンを追加します。" : "v51・緊急修正版";
  }
  ...
  function bindEvents() {
    ...
    els.resetView.addEventListener("click", () => map.setView(MIE_CENTER, 9));
  ...
  function init() {
    ...
    els.dataStatus.textContent = `v51・緊急修正 / 釣り場${state.spots.length}件 / 記録${state.catches.length}件 / 40up${state.catches.filter(isBigBass).length}件`;
  }
